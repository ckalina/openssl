...
static DWORD WINAPI thread_call_routine(LPVOID param)
{
    ...
    thread = (CRYPTO_THREAD_WIN*) param;
...
#ifndef __MINGW32__
    *thread->handle = CreateThread(NULL, 0, thread_call_routine, (LPVOID)thread,
                                   0, NULL);
#else
...
{
    DWORD retval_intern;
    CRYPTO_THREAD_WIN *thread_w;

    thread_w = (CRYPTO_THREAD_WIN*)thread;
    if (thread == NULL)
        return 0;
