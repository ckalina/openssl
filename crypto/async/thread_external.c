...
// @TODO #if defined(OPENSSL_THREADS) && !defined(CRYPTO_TDEBUG) && defined(OPENSSL_SYS_WINDOWS)
...
int CRYPTO_THREAD_EXTERN_enabled = 0;
...
CRYPTO_MUTEX   CRYPTO_THREAD_EXTERN_task_lock = NULL;
CRYPTO_CONDVAR CRYPTO_THREAD_EXTERN_task_cond_create = NULL;
CRYPTO_CONDVAR CRYPTO_THREAD_EXTERN_task_cond_finish = NULL;
...
int CRYPTO_THREAD_EXTERN_enable(CRYPTO_SIGNAL_PROPS* props)
{
    ...
    CRYPTO_THREAD_EXTERN_task_lock = mutex_create();
    CRYPTO_THREAD_EXTERN_task_cond_create = condvar_create();
    CRYPTO_THREAD_EXTERN_task_cond_finish = condvar_create();

    if (mutex_init(CRYPTO_THREAD_EXTERN_task_lock) == 0)
        goto fail;

    if (condvar_init(CRYPTO_THREAD_EXTERN_task_cond_create) == 0)
        goto fail;

    if (condvar_init(CRYPTO_THREAD_EXTERN_task_cond_finish) == 0)
        goto fail;
...
int CRYPTO_THREAD_EXTERN_disable(void)
{
    ...
    if (CRYPTO_THREAD_EXTERN_enabled) {
        mutex_destroy(CRYPTO_THREAD_EXTERN_task_lock);
        condvar_destroy(CRYPTO_THREAD_EXTERN_task_cond_create);
        condvar_destroy(CRYPTO_THREAD_EXTERN_task_cond_finish);
    }

    /* @TODO empty queues and free tasks */
...
static CRYPTO_THREAD_RETVAL CRYPTO_THREAD_EXTERN_worker(CRYPTO_THREAD_DATA data)
{
    size_t task_cnt;
    ...
    while (1) {
        ...
        list_del(job_l);
        mutex_unlock(CRYPTO_THREAD_EXTERN_task_lock);
...
static CRYPTO_THREAD CRYPTO_THREAD_EXTERN_provide(CRYPTO_THREAD_CALLBACK cb)
{
    return thread_create(CRYPTO_THREAD_EXTERN_worker, (CRYPTO_THREAD_DATA) cb);
}
...
CRYPTO_THREAD CRYPTO_THREAD_EXTERN_add_job(CRYPTO_THREAD_ROUTINE task, void* data)
{
    ...
    condvar_broadcast(CRYPTO_THREAD_EXTERN_task_cond_create);
    mutex_unlock(CRYPTO_THREAD_EXTERN_task_lock);
