                   ...
                   const void *key, size_t keylen)
{
    ...
    if ((ctx = EVP_MAC_new_ctx(mac)) == NULL)
        goto fail;
    ...
    if (EVP_MAC_set_ctx_params(ctx, par) != 1)
        goto fail;
                        ...
                        size_t inlen)
{
    ...
    if (!EVP_MAC_set_ctx_params(ctx, par))
        goto fail;
...
static void kdf_argon2_init(KDF_ARGON2 *ctx, ARGON2_TYPE type)
{
    ...
    c->type = type;
    c->openssl_ctx = PROV_LIBRARY_CONTEXT_OF(ctx->provctx);
}
