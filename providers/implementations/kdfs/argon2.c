...
# undef BLAKE2_ROUND_COLUMN
# define BLAKE2_ROUND_COLUMN(x, i)                                             \
    do {                                                                       \
        BLAKE2_ROUND(                                                          \
            x[16 * i],      x[16 * i + 1],  x[16 * i + 2],  x[16 * i + 3],     \
            x[16 * i + 4],  x[16 * i + 5],  x[16 * i + 6],  x[16 * i + 7],     \
            x[16 * i + 8],  x[16 * i + 9],  x[16 * i + 10], x[16 * i + 11],    \
            x[16 * i + 12], x[16 * i + 13], x[16 * i + 14], x[16 * i + 15]     \
        );                                                                     \
    } while ((void)0, 0)

# undef BLAKE2_ROUND_ROW
# define BLAKE2_ROUND_ROW(x, i)                                                \
    do {                                                                       \
        BLAKE2_ROUND(                                                          \
            x[2 * i],      x[2 * i + 1],  x[2 * i + 16],  x[2 * i + 17],       \
            x[2 * i + 32], x[2 * i + 33], x[2 * i + 48],  x[2 * i + 49],       \
            x[2 * i + 64], x[2 * i + 65], x[2 * i + 80],  x[2 * i + 81],       \
            x[2 * i + 96], x[2 * i + 97], x[2 * i + 112], x[2 * i + 113]       \
        );                                                                     \
    } while ((void)0, 0)
...
static void kdf_argon2_ctx_set_flags(ARGON2_CTX *ctx, uint32_t flags);
...
static uint32_t index_alpha(const ARGON2_CTX *ctx, uint32_t pass,
                            uint32_t lane, uint8_t slice, uint32_t index,
...
static int blake2b_long(unsigned char *out, size_t outlen, const void *in,
                        size_t inlen);
                       ...
                       block *next, int with_xor)
{
    ...
    for (i = 0; i < 8; ++i)
        BLAKE2_ROUND_COLUMN(blockR.v, i);

    for (i = 0; i < 8; i++)
        BLAKE2_ROUND_ROW(blockR.v, i);
                         ...
                         uint8_t slice, uint32_t index)
{
    ...
    for (j = start_idx; j < ctx->segment_length; ++j, ++curr_offset, ++prev_offset) {
        ...
        index = j;
        ref_index = index_alpha(ctx, pass, lane, slice, j, rnd & 0xFFFFFFFF,
                                ref_lane == lane);
        ...
        if(pass == 0)
            ...
            fill_block(ctx->memory + prev_offset, ref_block, curr_block, 1);
    }
}
...
static unsigned long CALLBACK fill_segment_thr(void *thread_data)
{
    ...
    fill_segment(my_data->ctx, my_data->pos.pass, my_data->pos.lane, my_data->pos.slice, my_data->pos.index);
    return 0;
}
...
static int fill_mem_blocks_mt(ARGON2_CTX *ctx) {
    ...
    t = OPENSSL_zalloc(sizeof(CRYPTO_THREAD)*ctx->lanes);
    ...
    for (r = 0; r < ctx->passes; ++r) {
        for (s = 0; s < ARGON2_SYNC_POINTS; ++s) {
            for (l = 0; l < ctx->lanes; ++l) {
                ARGON2_POS p;
                if (l >= ctx->threads)
                    if (CRYPTO_THREAD_join(t[l - ctx->threads], NULL) == 0)
                        goto fail;
                ...
                t[l] = CRYPTO_THREAD_new(&fill_segment_thr, (void*) &t_data[l]);

                if (t[l] == NULL) {
                    for (ll = 0; ll < l; ++ll)
                        CRYPTO_THREAD_join(t[ll], NULL);
                    CRYPTO_THREAD_clean(&t[ll]);
                    t[ll] = NULL;
                    goto fail;
                }
            }
            for (l = ctx->lanes - ctx->threads; l < ctx->lanes; ++l) {
                if (CRYPTO_THREAD_join(t[l], NULL) == 0)
                    goto fail;
                CRYPTO_THREAD_clean(&t[l]);
                t[l] = NULL;
            }
        }
    }
...
static int validate_inputs(const ARGON2_CTX *ctx)
{
    ...
    if (ctx->outlen < ARGON2_MIN_OUTLEN)
        return 0;
    if (ctx->outlen > ARGON2_MAX_OUTLEN)
        return 0;
    if (ctx->pwd == NULL && ctx->pwdlen != 0)
        return 0;
    if (ctx->pwdlen+1 < ARGON2_MIN_PWD_LENGTH+1)
        return 0;
    if (ctx->pwdlen > ARGON2_MAX_PWD_LENGTH)
        return 0;
    if (ctx->salt == NULL && ctx->saltlen != 0)
        return 0;
    if (ctx->saltlen < ARGON2_MIN_SALT_LENGTH)
        return 0;
    if (ctx->saltlen > ARGON2_MAX_SALT_LENGTH)
        return 0;
    if (ctx->secret == NULL && ctx->secretlen != 0)
        return 0;
    if (ctx->secret != NULL && ARGON2_MIN_SECRET+1 > ctx->secretlen+1)
        return 0;
    if (ctx->secret != NULL && ARGON2_MAX_SECRET < ctx->secretlen)
        return 0;
    if (ctx->ad == NULL && ctx->adlen != 0)
        return 0;
    if (ctx->ad != NULL && ARGON2_MIN_AD_LENGTH+1 > ctx->adlen+1)
        return 0;
    if (ctx->ad != NULL && ARGON2_MAX_AD_LENGTH < ctx->adlen)
        return 0;
    if (ctx->m_cost < ARGON2_MIN_MEMORY)
        return 0;
    if (ctx->m_cost >= (uint64_t) ARGON2_MAX_MEMORY)
        if (ctx->m_cost != (uint64_t) ARGON2_MAX_MEMORY)
            return 0;
    ...
    if (ctx->threads > 1)
        return 0;
#else
    if (CRYPTO_THREAD_enabled() == 0 && ctx->threads > 1)
        return 0;
#endif
...
static void initial_hash(uint8_t *blockhash, ARGON2_CTX *ctx)
{
    ...
    unsigned int tmp;
    uint32_t args[] = {
        ctx->lanes, ctx->outlen, ctx->m_cost, ctx->t_cost, ctx->version,
        (uint32_t) ctx->type, ctx->pwdlen
    };
...
static void finalize(const ARGON2_CTX *ctx)
{
    ...
    blake2b_long(ctx->out, ctx->outlen, blockhash_bytes, ARGON2_BLOCK_SIZE);
                   ...
                   const void *key, size_t keylen)
{
    ...
    if (EVP_MAC_CTX_set_params(ctx, par) != 1)
        goto fail;
                        ...
                        size_t inlen)
{
    ...
    if (outlen > UINT32_MAX || out == NULL || outlen == 0)
        goto fail;
    ...
    if (!EVP_MAC_CTX_set_params(ctx, par))
        goto fail;
    ...
    if (outlen <= BLAKE2B_OUTBYTES) {
        ...
        while(outlen_curr > BLAKE2B_OUTBYTES) {
            ...
            if (blake2b(outbuf, BLAKE2B_OUTBYTES, inbuf, BLAKE2B_OUTBYTES, NULL,
                        0) != 1)
                goto fail;
            memcpy(out, outbuf, BLAKE2B_OUTBYTES / 2);
            out += BLAKE2B_OUTBYTES / 2;
        ...
        memcpy(inbuf, outbuf, BLAKE2B_OUTBYTES);
        if (blake2b(outbuf, outlen_curr, inbuf, BLAKE2B_OUTBYTES, NULL, 0) != 1)
            goto fail;
